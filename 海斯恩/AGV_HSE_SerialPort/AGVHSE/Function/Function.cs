...
namespace TS_RGB.Fuction
{
    public class Function
    {
        ...
        public static int PR_UPDATE_AGVINFO_INITIAL(int lineNo, string waitNo)
        {
            SqlCommand comm = new SqlCommand("PR_UPDATE_AGVINFO_INITIAL");
            comm.CommandType = CommandType.StoredProcedure;
            comm.Parameters.Add("@LINENO", SqlDbType.Int).Value = lineNo;
            comm.Parameters.Add("@WAITNO", SqlDbType.VarChar, 50).Value = waitNo;
            try
            {
                return SqlDBControl.ExecuteNonQuery(comm);
            }
