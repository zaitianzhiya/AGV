...
namespace FileControl
{
    public class LogFile
    {
        ...
        private static string GetFilePath_PlcSerialPort()
        {
            string path = Application.StartupPath + @"\Log_PlcSerialPort" + @"\" + DateTime.Now.ToString("yyyy_MM_dd");
            if (!Directory.Exists(path))
            {
                Directory.CreateDirectory(path);
            }
            return path + @"\plcSerialPort.log";
        }
        ...
        /// <param name="path"></param>
        /// <param name="fileMsg"></param>
        public static void SavePlcSerialPortLog(string fileMsg)
        {
            try
            {
                using (FileStream _fStream = new FileStream(GetFilePath_PlcSerialPort(), FileMode.Append, FileAccess.Write))
                {
                    using (StreamWriter _sWrite = new StreamWriter(_fStream))
                    {
                        _sWrite.WriteLine(GetCurrentTimeString() + fileMsg);
